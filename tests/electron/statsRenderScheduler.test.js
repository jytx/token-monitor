...
test('a stats update repaints main and the visible Settings overlay', () => {
  ...
  // dev 多账号分支：minimax/deepseek/zai 三家走 apiKeyAccounts 通用层，
  // renderStatsUpdate 直接调 renderApiKeyAccountStatus 而非 external 路径。
  context.renderApiKeyAccountStatus = (provider) => calls.push(`apiKey:${provider}`);
