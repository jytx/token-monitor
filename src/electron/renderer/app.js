...
const { clientLabels } = window.TokenMonitorClientLabels;
...
function syncSettingsForm() {
  ...
  // macOS WidgetKit 小组件开关：仅 darwin 显示
  els.widgetRow?.classList.toggle('hidden', state.appInfo?.platform !== 'darwin');
  els.widgetInput.checked = state.settings.widgetEnabled !== false;
...
els.widgetInput?.addEventListener('change', () => {
  state.settings.widgetEnabled = els.widgetInput.checked;
  saveSettings({ widgetEnabled: els.widgetInput.checked });
});
