...
// 多账号路径：托管账号逐个探测（quota + subscription 固定两跳，与
// apiKeyAccounts 通用层的测试契约一致），行身份带账号键。main 新增的
// 余额 / ZCode 通道目前仅在单账号路径启用——按账号接入余额通道是 dev
// 的后续工作，不在本次合并中夹带。
async function fetchZaiManagedLimits(options, accounts, deps) {
  const region = zaiRegion(options, deps.env || process.env);
...
async function fetchZaiLimits(options = {}, deps = {}) {
  ...
  if (accounts.length > 0) return fetchZaiManagedLimits(options, accounts, deps);
